...
void *ThreadTrain (void *params) { // Code du thread
    ...
    for (int i = mes_param->start; i < mes_param->end; i++)
    {
       ...
       if(nb_traite==batch_size || i == (mes_param->end -1))
        {	
            gradient_descent(mes_param->rnn, mes_param->AVGgradient,  nb_traite, lr);
            nb_traite = 0;
        }
...
int main(int argc, char **argv)
{
    ...
    get_data(data, 2);
    double totaltime;
    int n , r, end, stop=1, e=0, start = 1 , size = 1000;
    ...
    initialize_rnn_gradient(rnn, AVGgradient);
    ...
    while ( (e < epoch) || (stop == 1) )
    {
        ...
        for(int t=0; t<NUM_THREADS; t++) 
        {
            ...
            somme_gradient(AVGgradient, threads_params[t].rnn);
        ...
        modelUpdate(rnn, AVGgradient, NUM_THREADS);
